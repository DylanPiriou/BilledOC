const mockedBills = {
	list(status = 200) {
		// Ajoutez un paramètre "status" pour déterminer le statut de réponse
		if (status === 404) {
			return Promise.reject({ status: 404, message: "Erreur 404" });
		} else if (status === 500) {
			return Promise.reject({ status: 500, message: "Erreur 500" });
		} else {
			// Simulez une réponse réussie avec des données factices
